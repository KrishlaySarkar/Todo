...
for s in subjects:
    while True:
        val = input(f"Marks for {s} (0-100): ").strip()
        try:
            m = int(val)
            if 0 <= m <= 100:
...
filename = f"{safe_name}_{roll}_marksheet.html"
path = os.path.abspath(filename)

# simple styled HTML
html = f"""<!doctype html>
<html>
<head><meta charset="utf-8"><title>Marksheet - {name}</title>
<style>
